...
func init() {
	flag.StringVar(&config.write, "w", "c", "c to create, u to update")
	flag.StringVar(&config.typeNames, "type", "", "comma-separated list of type names; must be set")
	flag.StringVar(&config.output, "output", "", "output file name; default wd/<wd>_consts.json")
}

func main() {
	log.SetFlags(0)
	log.SetPrefix("jsonconst: ")
	...
	switch config.write {
	case "u":
		bs, err := ioutil.ReadFile(path.Join(".", path.Base(wd)+"_consts.json"))
		if err != nil {
			log.Fatalf("ReadFile: %s", err)
		}
		err = json.Unmarshal(bs, &gen.result)
		if err != nil {
			log.Fatalf("Unmarshal File: %s", err)
		}
	case "c":
		gen.result = make(map[string][]string)
	default:
		flag.Usage()
		os.Exit(2)
	}
	...
	src, _ := json.MarshalIndent(gen.result, "", "  ")
	...
	case "":
		outFilename = path.Join(".", path.Base(wd)+"_consts.json")
	default:
